...
import { Mail, MapPin, Code, Star, Loader2, ArrowLeft, GraduationCap, Briefcase, Award, User, Phone, FileText, Link as LinkIcon, Github, Linkedin } from 'lucide-react';
...
export default function StudentPublicProfile() {
    ...
    const getFileUrl = (filePath) => {
        ...
        try {
            ...
            baseUrl = apiUrl.includes('/api/v1') ? apiUrl.replace('/api/v1', '') : apiUrl;
        }

        const normalizedPath = String(filePath).replace(/\\/g, '/');

        // If backend stored an absolute filesystem path, map it to the public static route.
        // Example: D:/.../backend/uploads/resumes/file.pdf -> /uploads/resumes/file.pdf
    ...
    const resumeUrl = getFileUrl(profile?.resume?.filePath);
                                ...
                                <div className="flex items-center gap-3 bg-gray-50 border border-gray-100 p-3 rounded-xl mt-2 w-max">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <div className="flex items-center gap-3 text-xs font-semibold">
                                            <a href={resumeUrl} target="_blank" rel="noopener noreferrer" className="text-primary-500 hover:text-primary-600 transition-colors">
                                                View
                                            </a>
                                            <span className="text-gray-300">|</span>
                                            <a href={resumeUrl} download={profile.resume.fileName || 'Resume.pdf'} target="_blank" rel="noopener noreferrer" className="text-primary-500 hover:text-primary-600 transition-colors">
                                                Download
                                            </a>
                                        </div>
                                    </div>
