...
import {
    ArrowLeft, Building, MapPin, Clock,
    Briefcase, Banknote, Star,
...
import { FormModal, AlertModal } from '@/components/common/Modal';
...
export default function InternshipDetailPage() {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            ...
            try {
                ...
                // 2. Fetch Match Analysis (non-blocking)
                try {
                ...
                const myAppsRes = await axios.get('/api/applications/me');
                if (myAppsRes.data.success) {
                    const existing = (myAppsRes.data.data || []).find(
    ...
    const handleApply = async () => {
        try {
            ...
            if (res.data.success) {
                setApplied(true);
                setApplicationId(res.data?.data?._id || null);
                setApplyModalOpen(false);
                setCoverLetter('');
                setAlert({
                    isOpen: true,
                    title: 'Application Submitted',
                    message: 'Your application was submitted successfully and is now in tracking.',
                    type: 'success'
                });
            }
        } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] p-6 lg:p-12 font-sans font-medium text-slate-600">
                        ...
                        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 space-y-6">
                            {isDisqualified ? (
                                ...
                                <div className="text-center space-y-4">
                                    ...
                                    <Link href={applicationId ? `/applications/${applicationId}` : '/applications'} className="block text-center text-[10px] font-black text-[#6366F1] uppercase tracking-widest hover:underline mt-4">
                                ...
                                <Button
                                    ...
                                    fullWidth
                                    variant="gradient"
                                    className="w-full group py-5 px-6 rounded-2xl font-black uppercase tracking-[0.2em] text-[11px] shadow-xl shadow-indigo-600/20"
                                >
            ...
            <FormModal
                ...
                submitText={applying ? 'Submitting...' : 'Submit'}
                loading={applying}
            >
                <div className="space-y-4">
                    <div>
                        ...
                        <textarea
                            ...
                            rows={6}
                            maxLength={2000}
                            placeholder="Add a short message to the employer..."
                        ...
                        />
                        <p className="text-xs text-gray-500 mt-1">{coverLetter.length}/2000</p>
                    </div>
                </div>
            </FormModal>
