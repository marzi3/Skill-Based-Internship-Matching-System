		...
		<div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
			<div className="h-40 relative group">
				{storedCoverImage ? (
					...
					<div className="w-full h-full bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center px-4">
						<p className="text-white/90 text-sm font-semibold text-center">No cover photo yet. Add one to make your profile stand out.</p>
					</div>
				)}
				{onEditCoverImage && (
					<button
						onClick={onEditCoverImage}
						className="absolute top-3 right-3 rounded-lg px-3 py-2 flex items-center justify-center gap-2 bg-white/95 text-slate-800 hover:bg-white transition-colors shadow-sm"
					>
						<Pencil size={14} />
						<span className="text-xs font-semibold">{storedCoverImage ? 'Change Cover Photo' : 'Add Cover Photo'}</span>
					</button>
				)}
			</div>
