...
const ProfileView = ({
	...
	return (
		<div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
			<div className="h-52 sm:h-60 relative group">
				{storedCoverImage ? (
					<img
					...
					<div className="w-full h-full bg-gradient-to-r from-slate-700 via-slate-500 to-slate-700" />
				)}
				<div className="absolute inset-0 bg-black/20" />
				{onEditCoverImage && (
					<button
						onClick={onEditCoverImage}
						className="absolute top-4 right-4 h-10 w-10 rounded-full bg-white/90 hover:bg-white shadow-md transition-all duration-200 flex items-center justify-center"
					>
			...
			<div className="relative px-6 sm:px-10 pb-6 pt-16 sm:pt-20">
				<div className="absolute -top-16 sm:-top-20 left-6 sm:left-10 w-28 h-28 sm:w-36 sm:h-36 rounded-full border-4 border-white bg-slate-200 flex items-center justify-center text-3xl font-bold text-slate-500 overflow-hidden shadow-lg group">
					{storedProfileImage ? (
					...
					{onEditProfileImage && (
						<button
							...
							className="absolute inset-0 bg-black/0 hover:bg-black/35 transition-all duration-200 rounded-full flex items-center justify-center"
						>
				...
				{onEditProfileImage && (
					<button
						onClick={onEditProfileImage}
						className="absolute top-4 right-4 h-10 w-10 rounded-full bg-white hover:bg-slate-50 shadow-md border border-slate-200 transition-all duration-200 flex items-center justify-center"
					>
						<Pencil size={16} className="text-slate-700" />
					</button>
				)}

				<div className="pl-0 sm:pl-44 mb-6 pt-2">
					<h3 className="text-2xl sm:text-4xl font-semibold text-slate-900 tracking-tight">
						{displayName}
					</h3>
					<p className="inline-block mt-3 px-4 py-1.5 rounded-full text-xs sm:text-sm font-semibold bg-amber-200 text-amber-900">
						{displayDesignation}
					</p>
					<p className="mt-3 text-sm text-slate-700">
						{user?.email || personalInfo.email || 'Not set'}
					</p>
					{personalInfo.about && (
